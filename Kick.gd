...
const KickHitbox = preload("res://KickHitbox.tscn")
...
func _on_StartupTimer_timeout():
	startup_timer.stop()
	SyncManager.spawn("KickHitbox", get_parent(), KickHitbox, { position = global_position, inverse = inverse, offset = sprite_width})
